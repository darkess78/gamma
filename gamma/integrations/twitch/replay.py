...
def replay_jsonl_text(
    ...
    *,
    ...
    stream_client = client or GammaStreamClient()
    results: list[dict[str, Any]] = []
    for raw in iter_replay_text(text):
        event = TwitchReplayEvent.model_validate(raw)
        stream_event = normalize_replay_event(event, owner_user_id=owner_user_id, session_id=session_id)
        results.append(
            stream_client.post_event(
...
def iter_replay_events(path: Path) -> Iterable[dict[str, Any]]:
    with path.open("r", encoding="utf-8") as handle:
        yield from iter_replay_lines(handle)


def iter_replay_text(text: str) -> Iterable[dict[str, Any]]:
    yield from iter_replay_lines(text.splitlines())


def iter_replay_lines(lines: Iterable[str]) -> Iterable[dict[str, Any]]:
