...
def test_optional_runtime_dependencies_are_not_in_the_core_install() -> None:
    ...
    assert {"openai", "faster-whisper", "sounddevice", "pillow", "pystray"}.isdisjoint(core)
    ...
    assert "pillow>=10.4.0" in extras["desktop"]
    assert "pystray>=0.19.5" in extras["desktop"]
...
def test_readme_documents_the_minimal_and_optional_install_paths() -> None:
    ...
    for extra in ("hosted", "local-voice", "desktop", "discord", "audio-understanding"):
